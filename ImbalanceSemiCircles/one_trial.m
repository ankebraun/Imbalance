...
%
% Presents two circular contracting/expanding gratings with changing contrast and then asks for which grating has stronger contrast.
%
% Parameters
% ----------
%
...
% duration : how long each contrast sample is shown in seconds
...
baseRect_left = [0 0 xpos(1) th];
baseRect_right = [xpos(2) 0 xpos(2)+xpos(1) th];

nrings = 2;
allRects = nan(4, nrings);
allRects(:,1) = CenterRectOnPointd(baseRect_left, xpos(1)-xpos(1)/2, yCenter);
allRects(:,2) = CenterRectOnPointd(baseRect_right, xpos(2)+xpos(1)/2, yCenter);
...
%    Screen('BlendFunction', window, 'GL_SRC_ALPHA', 'GL_ONE_MINUS_SRC_ALPHA');
    ...
    Screen('DrawTexture', window, ringtex, baseRect_left, allRects(:,1), [], [], [], low_left, [], [],...
       [high_left(1), high_left(2), high_left(3), high_left(4), shiftvalue, ringwidth, radius, inner_annulus, sigma, cutoff, xpos(1),yCenter]);

     Screen('DrawTexture', window, ringtex, baseRect_right, allRects(:,2), [], [], [], low_right, [], [],...
